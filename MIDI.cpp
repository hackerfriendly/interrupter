...
/*! \brief Read a MIDI message from the serial port using the main input channel
		...
		if (mPendingMessageIndex == 0) { // Start a new pending message
			...
			switch (getTypeFromStatusByte(mPendingMessage[0])) {
				...
				case SystemExclusive:
					mPendingMessageExpectedLength = MIDI_SYSEX_ARRAY_SIZE; // As the message can be any length between 3 and MIDI_SYSEX_ARRAY_SIZE bytes
					mRunningStatus_RX = InvalidType;
					break;
			...
			if (mPendingMessageIndex >= (mPendingMessageExpectedLength-1)) {
				...
				if (mPendingMessageExpectedLength == 3)	mMessage.data2 = mPendingMessage[2];
...
bool MIDI_Class::input_filter(byte inChannel)
{
	...
	if (mMessage.type >= NoteOff && mMessage.type <= PitchBend) {
		...
		if ((mMessage.channel == MIDI_CHANNEL_IN) || (mMessage.channel == MIDI_CHANNEL_OMNI)) {
			return true;
...
void MIDI_Class::thru_filter(byte inChannel)
{
	...
	if (mMessage.type >= NoteOff && mMessage.type <= PitchBend) {
		...
		const bool filter_condition = ((mMessage.channel == MIDI_CHANNEL_IN) || (mMessage.channel == MIDI_CHANNEL_OMNI));
